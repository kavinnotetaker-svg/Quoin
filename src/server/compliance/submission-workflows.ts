...
type WorkflowSummaryMap = {
  ...
  bepsByPacketId: Map<string, SubmissionWorkflowSummary>;
};
...
export async function reconcileBepsSubmissionWorkflowTx(
  tx: Prisma.TransactionClient,
  input: {
    organizationId: string;
    buildingId: string;
  ...
  return reconcileWorkflowForArtifactTx(tx, {
    ...
    filingPacketId: input.packet.id,
    artifact: input.packet,
    createdByType: input.createdByType,
    createdById: input.createdById ?? null,
    requestId: input.requestId ?? null,
...
export async function listSubmissionWorkflowSummariesForArtifacts(params: {
  ...
  filingPacketIds?: string[];
}) {
  const benchmarkPacketIds = Array.from(new Set(params.benchmarkPacketIds ?? [])).filter(Boolean);
  const filingPacketIds = Array.from(new Set(params.filingPacketIds ?? [])).filter(Boolean);

  if (benchmarkPacketIds.length === 0 && filingPacketIds.length === 0) {
    return {
      benchmarkByPacketId: new Map<string, SubmissionWorkflowSummary>(),
      bepsByPacketId: new Map<string, SubmissionWorkflowSummary>(),
  ...
  const workflows = await prisma.submissionWorkflow.findMany({
    where: {
      ...
      OR: [
        ...(benchmarkPacketIds.length > 0
          ? [{ benchmarkPacketId: { in: benchmarkPacketIds } }]
          : []),
        ...(filingPacketIds.length > 0
          ? [{ filingPacketId: { in: filingPacketIds } }]
          : []),
      ],
    },
    include: {
  ...
  const bepsByPacketId = new Map<string, SubmissionWorkflowSummary>();
  ...
  for (const workflow of workflows as WorkflowRecordWithRelations[]) {
    ...
    if (workflow.filingPacketId && !bepsByPacketId.has(workflow.filingPacketId)) {
      bepsByPacketId.set(workflow.filingPacketId, summary);
    }
  }
