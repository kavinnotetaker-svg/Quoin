...
function buildBaselinePayload(input: {
  ...
  return {
    ...
    governingContext: {
      ...
      filingYear: input.readiness.evaluations.beps?.filingYear ?? null,
      complianceCycle: toCycle(input.readiness.evaluations.beps?.complianceCycle),
      ruleVersion: input.complianceRun?.ruleVersion ?? input.readiness.evaluations.beps?.ruleVersion ?? null,
      factorSetVersion: input.complianceRun?.factorSetVersion ?? null,
      implementationKey: input.complianceRun?.implementationKey ?? null,
      metricUsed: input.complianceRun?.engineResult.metricUsed ?? input.readiness.evaluations.beps?.metricUsed ?? null,
    ...
    artifacts: {
      complianceRunId: input.complianceRun?.id ?? input.readiness.evaluations.beps?.complianceRunId ?? null,
      filingRecordId: input.readiness.artifacts.bepsFiling?.id ?? null,
      filingPacketId: input.readiness.artifacts.bepsPacket?.id ?? null,
    },
    timestamps: {
    ...
    keyDrivers: buildKeyDrivers({
      ...
      metricUsed:
        input.complianceRun?.engineResult.metricUsed ??
        input.readiness.evaluations.beps?.metricUsed ??
        null,
    }),
  };
}
...
function buildInputHash(input: {
  ...
  return hashDeterministicJson({
    ...
    readiness: {
      ...
      lastPacketGeneratedAt: input.readiness.lastPacketGeneratedAt,
      lastPacketFinalizedAt: input.readiness.lastPacketFinalizedAt,
      bepsFilingId: input.readiness.artifacts.bepsFiling?.id ?? null,
      bepsFilingStatus: input.readiness.artifacts.bepsFiling?.status ?? null,
      bepsPacketId: input.readiness.artifacts.bepsPacket?.id ?? null,
      bepsPacketStatus: input.readiness.artifacts.bepsPacket?.status ?? null,
...
export async function getOrCreatePenaltySummary(params: {
  ...
  try {
    ...
    const complianceRunContext = await loadComplianceRunPenaltyContext({
      organizationId: params.organizationId,
      buildingId: params.buildingId,
      complianceRunId: operationalState.readinessSummary.evaluations.beps?.complianceRunId ?? null,
    });
