...
import {
  ...
  listLatestPenaltyRunIdsByBuilding,
...
export type BulkPortfolioOperatorAction =
...
async function recordSkippedOperatorAction(input: {
  action:
    | "OPERATOR_PM_SYNC_RETRY_SKIPPED"
    | "OPERATOR_SOURCE_RECONCILIATION_REFRESH_SKIPPED"
    | "OPERATOR_PENALTY_REFRESH_SKIPPED";
  actorType: ActorType;
  actorId?: string | null;
  organizationId: string;
  buildingId: string;
...
export async function retryPortfolioManagerSyncFromOperator(input: {
  organizationId: string;
  buildingId: string;
  ...
  actorId?: string | null;
  requestId?: string | null;
  espmClient: Parameters<typeof syncPortfolioManagerForBuilding>[0]["espmClient"];
}) {
  ...
  try {
    const result = await syncPortfolioManagerForBuilding({
      organizationId: input.organizationId,
      buildingId: input.buildingId,
      reportingYear: input.reportingYear,
      espmClient: input.espmClient,
      producedByType: input.actorType,
      producedById: input.actorId ?? null,
      requestId: input.requestId ?? null,
    });
    ...
    await createAuditLog({
      ...
      outputSnapshot: {
        ...
        latestJobId: result.syncState.latestJobId ?? null,
      },
    });

    return {
      action: "PORTFOLIO_MANAGER_SYNC",
    ...
    await createAuditLog({
      ...auditBase,
      action: "OPERATOR_PM_SYNC_RETRY_FAILED",
...
export async function refreshPenaltySummaryFromOperator(input: {
  organizationId: string;
  buildingId: string;
  actorType: ActorType;
  actorId?: string | null;
  requestId?: string | null;
  previousPenaltyRunId?: string | null;
}) {
  const auditBase = buildOperatorActionAuditBase(input);

  await createAuditLog({
    ...auditBase,
    action: "OPERATOR_PENALTY_REFRESH_REQUESTED",
  });

  try {
    const summary = await getOrCreatePenaltySummary({
      organizationId: input.organizationId,
      buildingId: input.buildingId,
      requestId: input.requestId ?? null,
    });
    const reused = input.previousPenaltyRunId === summary.id;

    await createAuditLog({
      ...auditBase,
      action: "OPERATOR_PENALTY_REFRESH_COMPLETED",
      outputSnapshot: {
        penaltyRunId: summary.id,
        status: summary.status,
        currentEstimatedPenalty: summary.currentEstimatedPenalty,
        reused,
      },
    });

    return {
      action: "PENALTY_REFRESH",
      status: reused ? "REUSED" : "COMPUTED",
      penaltyRunId: summary.id,
      currentEstimatedPenalty: summary.currentEstimatedPenalty,
      calculatedAt: summary.calculatedAt,
      message: reused
        ? "The latest governed penalty run is already current for this building."
        : "A new governed penalty run was recorded.",
    };
  } catch (error) {
    await createAuditLog({
      ...auditBase,
      action: "OPERATOR_PENALTY_REFRESH_FAILED",
      errorCode: error instanceof Error ? error.name : "UNKNOWN_ERROR",
    });
    throw error;
  }
...
export async function executeBulkPortfolioOperatorAction(input: {
  ...
  const previousPenaltyRunIdsByBuilding =
    input.action === "REFRESH_PENALTY_SUMMARY"
      ? await listLatestPenaltyRunIdsByBuilding({
          organizationId: input.organizationId,
  ...
  for (const buildingId of requestedBuildingIds) {
    ...
    try {
      switch (input.action) {
        ...
        case "REFRESH_PENALTY_SUMMARY": {
          const result = await refreshPenaltySummaryFromOperator({
            organizationId: input.organizationId,
            buildingId: building.id,
            actorType: input.actorType,
            actorId: input.actorId ?? null,
            requestId: input.requestId ?? null,
            previousPenaltyRunId:
              previousPenaltyRunIdsByBuilding.get(building.id) ?? null,
          });
        ...
        case "RETRY_PORTFOLIO_MANAGER_SYNC": {
          if (!building.espmPropertyId) {
            const reason =
              "Portfolio Manager sync is not configured for this building.";
            await recordSkippedOperatorAction({
          ...
          if (!input.espmClient) {
            throw new ValidationError(
              "Portfolio Manager sync retry requires an ESPM client.",
            );
          }

          const result = await retryPortfolioManagerSyncFromOperator({
            organizationId: input.organizationId,
            buildingId: building.id,
            reportingYear: input.reportingYear,
            actorType: input.actorType,
          ...
          results.push({
            ...
            message:
              result.status === "FAILED"
                ? "Portfolio Manager sync completed with a failed governed runtime state."
                : result.message,
          });
          break;
        }
